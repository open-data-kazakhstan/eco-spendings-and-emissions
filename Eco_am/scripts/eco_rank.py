...
file = r'C:\Users\USER\Desktop\Eco_project\Eco_am\data\unpiv.csv'
df_1 = pd.read_csv(file)

file2 = r'C:\Users\USER\Desktop\Eco_project\Eco_am\data\fin_eco_am.csv'
...
def ml_new_regions(region_to_test, region_predict):
    ...
    values = df_vko['Выделенный бюджет на защиту экологии'].values.tolist()
    k = 0
    c = 0
    ...
    list_res = []
    list_yrs = []
    filtered_df = df.loc[(df['Регион'] == region_predict) & (df['Год'] == 2021), 'Выделенный бюджет на защиту экологии']
    ...
    kval = 0
    for i in list_yrs:
        df['Выделенный бюджет на защиту экологии'] = np.where((df['Регион'] == region_predict) & (df['Год'] == i), list_res[kval], df['Выделенный бюджет на защиту экологии'])
        kval = kval +1
...
li = []
ranks = []
df1 = df1.sort_values(by=['Год', 'Регион', 'Выделенный бюджет на защиту экологии'], ascending=[True, True, True])
for i in df1['Регион']:
    if i not in li:
        ranks.append(1)
        li.append(i)
...
df1['Выделенный бюджет на защиту экологии'] = df1['Выделенный бюджет на защиту экологии'].fillna(0).astype('int')
df1['Год'] = df1['Год'].fillna(0).astype('int')
plt.style.use('dark_background')
fig, ax = plt.subplots(figsize=(15,8))
df1.to_csv(r'C:\Users\USER\Desktop\Eco_project\Eco_am\data\eco_rank.csv') 